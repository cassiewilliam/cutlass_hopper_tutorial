        ...
        for (int j = 0; j < N; j += 64)
        {
            ...
            for (int k = 0; k < K; k += 32)
            {
                for (int kk = 0; kk < 32; ++kk)
                {
                    ...
                    for (int jj = 0; jj < 64; ++jj)
                    {
                        ...
                        {
                            accum[ii * 64 + jj] += ((AccumType)hA[(i + ii) % M * K + k + kk] *
                                                    (AccumType)hB[(j + jj) % N * K + k + kk]);
                        }
                    }
                }
            }
            for (int ii = 0; ii < 64; ++ii)
            {
                for (int jj = 0; jj < 64; ++jj)
                {
                    if (i + ii < M && j + jj < N)
                    {
                        golden[(i + ii) * N + j + jj] = (CType)(accum[ii * 64 + jj] * params.alpha +
                                                                params.beta);
                    }
                }
            }
...
template<class AType, class BType, class CType, class AccumType, int MTile, int NTile, int KTile>
__global__ void reference_gpu_gemm_kernel(GemmParams<AType, BType, CType, AccumType> params)
{
    ...
    for (int k = 0; k < params.K; k += KTile)
    {
        for (int mi = 0; mi < MTile; ++mi)
        {
            for (int ki = 0; ki < KTile; ++ki)
            {
                ...
                {
                    AType aval = k + ki < params.K
                                     ? params.A[(m + mi) % params.M * params.K + k + ki]
                                     : AType(0);
                    BType bval = k + ki < params.K
                                     ? params.B[(n + ni) % params.N * params.K + k + ki]
                                     : BType(0);
    ...
    for (int mi = 0; mi < MTile; ++mi)
    {
        for (int ni = 0; ni < NTile; ++ni)
        {
            ...
            {
                params.C[(m + mi) * params.N + n + ni] = (CType)(accum[mi * NTile + ni] *
                                                                     params.alpha +
                                                                 params.beta);
            }
        }
...
void reference_gpu_gemm(GemmParams<AType, BType, CType, AccumType> params)
{
    ...
    dim3      block(tx, ty);
    reference_gpu_gemm_kernel<AType, BType, CType, AccumType, mTile, nTile, kTile>
        <<<grid, block>>>(params);
