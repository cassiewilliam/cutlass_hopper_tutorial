...
int get_sm_count()
{
    ...
    {
        std::cerr << "cudaGetDevice() returned error " << cudaGetErrorString(result) << std::endl;
        return 1;
    }
    ...
    if (result != cudaSuccess)
    {
        std::cerr << "cudaDeviceGetAttribute() returned error " << cudaGetErrorString(result)
                  << std::endl;
        return 1;
    }
...
HOST_DEVICE constexpr int ceil_div(int a, int b)
{
    return (a + (b - 1)) / b;
}
...
struct CPUTimer : public Timer
{
    ...
    {
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(trace_[1 - cur_] -
                                                                              trace_[cur_]);
...
{
    return (DType*)malloc(std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int>()) *
                          sizeof(DType));
}
...
DType* alloc_gpu_tensor(std::vector<int> shape)
{
    ...
    CUDA_CHECK(cudaMalloc(
        (void**)&dt,
        std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int>()) * sizeof(DType)));
    return dt;
}
...
template<class DType>
void copy_to_gpu_async(DType* hptr, DType* dptr, std::vector<int> shape, cudaStream_t stream = 0)
{
...
template<class DType>
void copy_to_cpu_async(DType* hptr, DType* dptr, std::vector<int> shape, cudaStream_t stream = 0)
{
    CUDA_CHECK(cudaMemcpyAsync(
        hptr,
        dptr,
...
template<class DType>
void assert_allclose(DType*           res_ptr,
                     DType*           golden_ptr,
