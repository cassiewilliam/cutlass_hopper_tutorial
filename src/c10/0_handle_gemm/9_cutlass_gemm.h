...
inline CUtensorMapDataType to_CUtensorMapDataType()
{
    ...
    */
    ...
    /*
    else if constexpr (std::is_same<T, cutlass::bfloat16_t>::value)
    {
        return CU_TENSOR_MAP_DATA_TYPE_BFLOAT16;
    }
    else if constexpr (std::is_same<T, cutlass::tfloat32_t>::value)
    {
...
TmaDescriptor make_tma_copy_desc(DType*                  gmem_ptr,
                                 int                     shape_major,
                                 int                     shape_minor,
                                 ...
                                 bool                    verbose = false)
{
    ...
    uint32_t smem_box_shape[5]  = {uint32_t(BlockMinorSize),
                                   uint32_t(BlockMajorSize / num_multicast),
                                   1,
                                   1,
                                   1};
                        ...
                        int32_t const&             crd1)
{
    ...
    asm volatile("cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::"
                 "bytes"
                 " [%0], [%1, {%3, %4}], [%2];"
                 :
                                  ...
                                  int32_t const&             crd2)
{
    ...
    uint32_t smem_int_mbar = cast_smem_ptr_to_uint(&smem_mbar);
    uint32_t smem_int_ptr  = cast_smem_ptr_to_uint(smem_ptr);
    asm volatile("cp.async.bulk.tensor.3d.shared::cluster.global.mbarrier::complete_tx::"
                 "bytes.multicast::cluster"
                 " [%0], [%1, {%4, %5, %6}], [%2], %3;"
                 :
                 : "r"(smem_int_ptr),
...
{
    DEVICE static void wgmma(uint64_t const& desc_a,
...
{
    HOST_DEVICE constexpr GmmaDescriptor() noexcept
        : desc_(0)
    {}
    HOST_DEVICE constexpr GmmaDescriptor(uint64_t desc) noexcept
        : desc_(desc)
    {}
    ...
    HOST_DEVICE constexpr GmmaDescriptor& operator=(GmmaDescriptor const& t) noexcept
    {
        desc_ = t.desc_;
        return *this;
    }
    ...
    // Decay to a uint64_t
    HOST_DEVICE constexpr operator uint64_t() const noexcept
    {
        return desc_;
    }
};
...
DEVICE GmmaDescriptor make_smem_desc(PointerType smem_ptr)
{
    ...
    desc.bitfield.layout_type_         = 0x1;   /// swizzle 128B because we use Swizzle<3,4,3>
    desc.bitfield.leading_byte_offset_ = 0x1;   /// no use
    desc.bitfield
        .stride_byte_offset_   = 64;   /// how many 128bits-rows needed between two core matrices
...
/**
 * Cluster-wide barrier。CUDA的屏障机制不支持集群级操作，因此需遵循CUTLASS框架的实现方案。
 * 具体而言，CUDA未实现集群范围屏障的核心原因在于屏障到达操作（barrier
arrive）无法返回阶段令牌（phase token）。
 * 为此，CUTLASS选择不将阶段令牌作为返回值使用，但在同步等待（wait）操作中仍需依赖该令牌完成协调
┌────────────────────────────┐
│  init(arrive_count)        │ ◀─ 初始化 mbarrier，到达计数设置
│  初始化 barrier 状态        │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  arrive()                  │ ◀─ 当前线程块内线程通知已到达
│  本地通知 barrier          │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  arrive(cta_id)            │ ◀─ 远程线程块到达通知（跨 CTA）
│  cluster 范围内的 arrive   │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  wait(phase)               │ ◀─ 等待所有线程到达后 phase 翻转
│  阻塞等待特定 phase        │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  try_wait(phase)           │ ◀─ 非阻塞方式尝试等待完成
│  返回是否满足等待条件      │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  invalidate()              │ ◀─ 使 barrier 状态无效
│  释放或重置 barrier        │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  arrive_and_expect_tx()    │ ◀─ 到达 + 声明预期的数据传输量
│  用于 TMA 预告传输          │
└────────────────────────────┘
             │
             ▼
┌────────────────────────────┐
│  expect_transaction()      │ ◀─ 表明将会消费一定字节数据
│  通常与 TMA 通信结合使用    │
└────────────────────────────┘
 */
struct Barrier
{
    ...
    DEVICE void arrive_and_expect_tx(uint32_t transaction_bytes, uint32_t cta_id, uint32_t pred)
    {
        uint32_t smem_addr = cast_smem_ptr_to_uint(&barrier_);
        asm volatile("{\n\t"
                     ".reg .pred p;\n\t"
...
struct PipelineState
{
    ...
    DEVICE PipelineState advance(uint32_t num_iterations)
    {
        if constexpr (Stages > 0)
        {
            ...
            if ((num_iterations >= Stages) && (((index + num_iterations) / Stages) % 2) == 1)
            {
                phase ^= 1;
            }
...
struct TmaPipeline
{
    ...
    DEVICE TmaPipeline(TmaPipelineSharedStorage<Stages>& storage, TmaPipelineParams<Stages> p)
        ...
        , params(p)
    {
        ...
        if (warp_idx == 0 && lane_predicate == 1)
        {
            ...
            uint32_t const num_consumer_warpgroups_per_cluster = params.num_consumers /
                                                                 WARP_GROUP_SIZE;
            // Question: why this? I guess it's the same row and col.
            uint32_t const multicast_consumer_arrival_count = (ClusterM + ClusterN - 1) *
        ...
        if (params.num_consumers % WARP_GROUP_SIZE == 0)
        {
            ...
            is_signaling_thread = (thread_idx % (WARP_GROUP_SIZE / MAX_CLUSTER_SIZE)) == 0;
                                 ...
                                 ProducerToken         barrier_token = {BarrierStatus::WaitAgain})
    {
        ...
        if (params.is_leader)
        {
            full_barrier_ptr[state.index].arrive_and_expect_tx(params.transaction_bytes);
        }
    }
    ...
    DEVICE ConsumerToken consumer_try_wait(PipelineState<Stages> state, uint32_t skip_wait = false)
    {
        if (skip_wait)
        {
            return {BarrierStatus::WaitDone};
        }
        uint32_t barrier_status = full_barrier_ptr[state.index].try_wait(state.phase);
    ...
    DEVICE void consumer_wait(PipelineState<Stages> state, ConsumerToken barrier_token)
    {
    ...
    DEVICE void consumer_release(PipelineState<Stages> state, uint32_t skip = false)
    {
        empty_barrier_ptr[state.index].arrive(dst_blockid, is_signaling_thread & (!skip));
    }
};
...
struct EpilogueLoadPipeline
{
    ...
    DEVICE EpilogueLoadPipeline(EpilogueLoadPipelineSharedStorage<Stages>& storage,
                                EpilgoueLoadPipelineParams<Stages>&        params)
        : full_barrier_ptr(&storage.full_barrier[0])
        , empty_barrier_ptr(&storage.empty_barrier[0])
        , params(params)
...
struct WgMMA<half_t, half_t, float, MTile, NTile, KTile>
{
    ...
    static constexpr int num_elements_accumulators = num_warp_groups_m * warp_repeats_per_row *
                                                     warp_repeats_per_col * elements_per_thread;
    ...
    DEVICE static void get_m_n_idx_fragment(int& m,
                                            int& n,
                                            int  thread_id,
                                            int  k_wgmma,
                                            int  row_id,
                                            ...
                                            int  item_id)
    {
        ...
        n = col_id * warp_elements_per_row + lane_id % threads_per_row * elements_per_thread +
            item_id;
    }

    DEVICE static void
    get_4d_idx_from_linear(int& k_wgmma, int& row_id, int& col_id, int& item_id, int linear_id)
    {
        item_id = linear_id % elements_per_thread;
        row_id  = linear_id / elements_per_thread % warp_repeats_per_col;
        col_id  = linear_id / elements_per_thread / warp_repeats_per_col % warp_repeats_per_row;
    ...
    DEVICE static void wgmma(half_t* smem_A, half_t* smem_B, float* accum)
    {
        float* accum_ = accum;
        {
            ...
            for (int m = 0; m < num_warp_groups_m; ++m)
            {
                ...
                auto desc_a = utils::make_smem_desc(smem_A + k * WGMMA_K + m * WGMMA_M * KTile);
                // the first is ScaleD
                utils::SM90_64x128x16_F32F16F16_SS<ScaleA, ScaleB, ScaleD, TransA, TransB>::wgmma(
                    desc_a,
                    desc_b,
                    accum[0],
                    accum[1],
        ...
        for (int k = 1; k < k_iter; ++k)
        {
            ...
            for (int m = 0; m < num_warp_groups_m; ++m)
            {
                ...
                utils::SM90_64x128x16_F32F16F16_SS<ScaleA, ScaleB, 1, TransA, TransB>::wgmma(
                    desc_a,
                    desc_b,
                    accum[0],
                    accum[1],
...
struct TileScheduler
{
    ...
    DEVICE TileScheduler(int M, int N)
    {
        init(M, N);
    }
    ...
    DEVICE void get_current_m_n_idx(int& m_idx, int& n_idx, int m_blocks, int n_blocks)
    {
...
struct Mainloop
{
    ...
    static constexpr uint32_t TmaTransactionBytes = BlockM * BlockK * sizeof(AType) +
                                                    BlockN * BlockK * sizeof(BType);

    DEVICE static void prefetch_tma_descriptor(const utils::TmaDescriptor* tensormap_a,
                                           ...
                                           Stages>&          shared_storage)
    {
        ...
        if (warp_idx_in_warp_group == 0 && lane_predicate == 1)
        {
            ...
            if constexpr (ClusterM > 1)
            {
                ...
                {
                    mcast_mask_b |= (uint16_t(1) << (block_id_y_in_cluster * ClusterM + i));
                }
            }
            if constexpr (ClusterN > 1)
            {
                ...
                {
                    mcast_mask_a |= (uint16_t(1) << (block_id_x_in_cluster + i * ClusterN));
                }
            }
                    ...
                    utils::tma_copy_3d_multicast(
                        ...
                        *mainloop_pipeline.producer_get_barrier(mainloop_pipeline_state),
                        ...
                        reinterpret_cast<void*>(smem_ptr_A + block_id_y_in_cluster *
                                                                 multicast_stride_a * BlockK),
                        i * BlockK,   // innermost dim moves fastest
                        m_idx * BlockM + block_id_y_in_cluster * multicast_stride_a,
                        0);
                }
                else
                {
                    ...
                    utils::tma_copy_3d_multicast(
                        ...
                        *mainloop_pipeline.producer_get_barrier(mainloop_pipeline_state),
                        ...
                        reinterpret_cast<void*>(smem_ptr_B + block_id_x_in_cluster *
                                                                 multicast_stride_b * BlockK),
                        i * BlockK,   // innermost dim moves fastest
                        n_idx * BlockN + block_id_x_in_cluster * multicast_stride_b,
                        0);
    ...
    DEVICE void load_tail(TmaPipeline<Stages, ClusterM, ClusterN> mainloop_pipeline,
                          PipelineState<Stages>                   mainloop_pipeline_state)
    {
        int warp_idx               = threadIdx.x / WARP_SIZE;
        int warp_idx_in_warp_group = warp_idx % 4;
    ...
    {
        PipelineState<Stages> mainloop_pipeline_state_release = mainloop_pipeline_state;
        ...
        WGMMA::wgmma<1, 1, 0, 0, 0>(
            shared_tensors.smem_A + read_stage * BlockM * BlockK,   // half_t* smem_A,
            shared_tensors.smem_B + read_stage * BlockN * BlockK,   // half_t* smem_B,
            accum                                                   // float* accum,
        );
    ...
    DEVICE void mma_tail(TmaPipeline<Stages, ClusterM, ClusterN> mainloop_pipeline,
                         PipelineState<Stages>                   mainloop_pipeline_state,
                         int                                     k_tile_count)
    {
...
#pragma unroll
        for (int i = 0; i < WGMMA::num_elements_accumulators; ++i)
        {
            ...
            WGMMA::get_m_n_idx_fragment(m_frag,
                                        n_frag,
                                        threadIdx.x % WARP_GROUP_SIZE,
                                        k_wgmma,
                                        row_id,
                                        col_id,
                                        item_id);
...
using MathWarpGroupOrderBarrierSharedStorage = OrderedBarrierSharedStorage<2, 2>;
...
{
    alignas(128) MainloopSharedStorage<AType,
                                       BType,
                                       CType,
                                       AccumType,
                                       BlockM,
    ...
    const __grid_constant__ utils::TmaDescriptor tensormap_b)
{
    ...
    KernelSharedStorage<AType,
                        ...
                        ClusterM,
                        ClusterN,
                        Stages>& shared_storage = *reinterpret_cast<KernelSharedStorage<AType,
                                                                                        BType,
                                                                                        CType,
                                                                                        AccumType,
                                                                                        BlockM,
    ...
    int      warp_idx               = threadIdx.x / WARP_SIZE;
    int      warp_idx_in_warp_group = threadIdx.x / WARP_SIZE % WARP_NUMBER_IN_WARP_GROUP;
    ...
    {
        mainloop_pipeline_params.role = TmaPipelineParams<Stages>::ThreadCategory::Producer;
    }
    if (warp_group_role == WarpGroupRole::Consumer0 || warp_group_role == WarpGroupRole::Consumer1)
    {
        mainloop_pipeline_params.role = TmaPipelineParams<Stages>::ThreadCategory::Consumer;
    }
    mainloop_pipeline_params.is_leader         = warp_group_thread_idx == 0;
    mainloop_pipeline_params.num_consumers     = WARP_GROUP_SIZE;
    mainloop_pipeline_params.transaction_bytes = Mainloop<AType,
                                                          BType,
                                                          CType,
                                                          AccumType,
                                                          BlockM,
                                                          ...
                                                          ClusterM,
                                                          ClusterN,
                                                          Stages>::TmaTransactionBytes;
    TmaPipeline<Stages, ClusterM, ClusterN> mainloop_pipeline(shared_storage.mainloop.pipeline,
    ...
    load_order_params.group_id   = producer_warp_role == ProducerWarpRole::Mainloop ? 0 : 1;
    ...
    MathWarpGroupOrderBarrier math_wg_order(shared_storage.math_wg_order, math_wg_order_params);

    PipelineState<Stages> mainloop_pipeline_consumer_state;
    PipelineState<Stages> mainloop_pipeline_producer_state = make_producer_start_state<Stages>();

    Mainloop<AType, BType, CType, AccumType, BlockM, BlockN, BlockK, ClusterM, ClusterN, Stages>
        mainloop;

    WgMMA<AType, BType, AccumType, BlockM, BlockN, BlockK> wgmma;
    Epilogue<AType, BType, CType, AccumType, BlockM, BlockN, BlockK, ClusterM, ClusterN, Stages>
        epilogue;
    ...
    TileScheduler<BlockM, BlockN, ClusterM, ClusterN> scheduler(kernel_params.gemm_params.M,
                                                                kernel_params.gemm_params.N);
    ...
    if (warp_group_role == WarpGroupRole::Producer)
    {
        ...
        if (producer_warp_role == ProducerWarpRole::Mainloop)
        {
            ...
            while (work_tile_info.valid)
            {
                ...
                mainloop.load(tensormap_a,
                              tensormap_b,
                              mainloop_pipeline,
                              mainloop_pipeline_producer_state,
                              work_tile_info.m_idx,
                              work_tile_info.n_idx,
                              k_tile_count,
             ...
             warp_group_role == WarpGroupRole::Consumer1)
    {
        ...
        while (work_tile_info.valid)
        {
            ...
            // work_tile_info.n_idx);
            AccumType accumulators
                [WgMMA<AType, BType, AccumType, BlockM, BlockN, BlockK>::num_elements_accumulators];

            // consuemr 0 doens't block at the first wait
            math_wg_order.wait();

            mainloop.mma(mainloop_pipeline,
            ...
            epilogue.store(kernel_params.gemm_params.C,
                           wgmma,
                           accumulators,
                           work_tile_info.m_idx,
                           work_tile_info.n_idx,
                           kernel_params.gemm_params.M,
                           kernel_params.gemm_params.N);
...
void gpu_gemm(GemmParams<AType, BType, CType, AccumType> gemm_params, bool verbose = true)
{
    ...
    auto*  Kernel        = gpu_gemm_kernel<AType,
                                           BType,
                                           CType,
                                           AccumType,
                                           BLOCKM,
                                           ...
                                           CLUSTER_N,
                                           STAGES>;
    size_t smemSizeBytes = sizeof(KernelSharedStorage<AType,
                                                      BType,
                                                      CType,
                                                      AccumType,
                                                      BLOCKM,
    ...
    {
        cudaError_t result = cudaFuncSetAttribute(Kernel,
    ...
    if (verbose)
    {
        std::cout << "Launching kernel with grid " << grid.x << " " << grid.y << " " << grid.z
                  << " and block " << block.x << " " << block.y << " " << block.z << " and cluster "
                  << cluster.x << " " << cluster.y << " " << cluster.z << " and smem "
                  << smemSizeBytes << " bytes\n";
    }
    void const* kernel = (void const*)Kernel;

    cudaError_t status = cudaFuncSetAttribute(kernel,
                                              cudaFuncAttributeNonPortableClusterSizeAllowed,
                                              1);
    CUDA_CHECK(status);

    // tensor_map
    utils::TmaDescriptor tensormap_a = utils::make_tma_copy_desc<BLOCKM, BLOCKK, 3>(
        gemm_params.A,
        gemm_params.M,
        gemm_params.K,
        Swizzle<3, 4, 3>{},
        CLUSTER_N,
        verbose);
    utils::TmaDescriptor tensormap_b = utils::make_tma_copy_desc<BLOCKN, BLOCKK, 3>(
        gemm_params.B,
        gemm_params.N,
        gemm_params.K,
        Swizzle<3, 4, 3>{},
