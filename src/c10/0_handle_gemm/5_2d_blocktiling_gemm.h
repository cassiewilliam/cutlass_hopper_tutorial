...
template<const int kBlockM,
         ...
         const int kThreadM,
         const int kThreadN>
__global__ void sgemm_2d_blocktiling(int          M,
                                     int          N,
                                     int          K,
                                     float        alpha,
                                     const float* A,
                                     ...
                                     float*       C)
{
    ...
    const uint total_results_block_tile = kBlockM * kBlockN;
    // 每个线程计算一个二维的(kThreadM, kThreadN)块，总共kThreadM * kThreadN的元素
    const uint num_threads_block_tile = total_results_block_tile / (kThreadM * kThreadN)

    // 每个Block的线程数要等于一个Block对应的线程数量
    assert(num_threads_block_tile == blockDim.x);
...
void run_sgemm_2d_blocktiling(int    M,
                              int    N,
                              int    K,
                              float  alpha,
                              float* A,
                              ...
                              float* C)
{
    ...
    sgemm_2d_blocktiling<32><<<grid_dim, block_dim>>>(M, N, K, alpha, A, B, beta, C);
}
