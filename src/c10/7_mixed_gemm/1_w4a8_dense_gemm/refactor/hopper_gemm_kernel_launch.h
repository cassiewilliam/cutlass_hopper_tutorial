...
void hopper_w4a8_dense_gemm(
    ...
    int64_t              stride_result,
    const float*         alpha,
    const float*         beta,
    const __nv_fp8_e4m3* input,
    const uint8_t*       weight,
    const float*         input_per_channel_scales,   // per channel quant scale, with shape [k]
    const float*         input_fp8_scale,            // per tensor quant scale, with shape [1]
    const float* weight_per_group_scales,   // per group quant scale, with shape[n, k / group_size]
    const float* weight_fp8_scale,          // per tensor quant scale, with shape [1]
    cudaStream_t stream)
{
    // fp32 accum optimal sizes
    using KernelTraits = MixedInputGemmKernelTraits<OutType>;
    ...
    using Scheduler = std::conditional_t<validation, cfx::SingleTileScheduler, TileScheduler>;

    typename CollectiveMainloop::Params
        mainloop_params = CollectiveMainloop::to_underlying_arguments({
            A,
    ...
    typename CollectiveEpilogue::Params
        epilogue_params = CollectiveEpilogue::to_underlying_arguments(
            {C, make_layout(make_shape(M, N), make_stride(ldC, Int<1>{}))});
    ...
    num_blocks_m = cutlass::ceil_div(num_blocks_m, size<0>(ClusterShape{})) *
                   size<0>(ClusterShape{});
    num_blocks_n = cutlass::ceil_div(num_blocks_n, size<1>(ClusterShape{})) *
                   size<1>(ClusterShape{});
    ...
    typename Scheduler::Arguments scheduler_args   = {num_blocks_m,
                                                      num_blocks_n,
                                                      num_blocks_k,
                                                      workspace};
    typename Scheduler::Params    scheduler_params = Scheduler::to_underlying_arguments(
        scheduler_args);
    ...
    cutlass::ClusterLaunchParams launch_params{grid_dims,
                                               block_dims,
                                               cluster_dims,
                                               smem_size,
                                               stream};
    ...
    cutlass::launch_kernel_on_cluster(launch_params,
                                      kernel,
                                      mainloop_params,
                                      epilogue_params,
                                      scheduler_params);
    ...
    if (iters > 0)
    {
        ...
        {
            Scheduler::initialize_workspace(num_blocks_m * num_blocks_n,
                                            num_sms,
                                            workspace,
                                            stream);
...
void dispatch_on_scheduler(int          M,
                           int          N,
                           int          K,
                           TI           alpha,
                           TA const*    A,
                           ...
                           cudaStream_t stream = 0)
{
    ...
    {
    case 0:
        gemm_tn<TA, TB, TC, TI, TI, cfx::SingleTileScheduler, validation, print_layouts_only>(
            M,
            N,
            K,
            alpha,
    ...
    case 1:
        gemm_tn<
            ...
            TI,
            TI,
            cfx::DataParallelPersistentTileScheduler,
            validation,
            print_layouts_only>(M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC, iters, csv, stream);
        break;
    case 2:
        using KernelTraits = Kernel_traits<256,
                                           192,
                                           128,
                                           12,
                                           2,
        ...
        gemm_tn<
            ...
            TI,
            TI,
            cfx::StreamKPersistentTileScheduler<KernelTraits>,
            validation,
            print_layouts_only>(M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC, iters, csv, stream);
        break;
    case 3:
        ...
        gemm_tn<
            ...
            TI,
            TI,
            cfx::StreamKPersistentTileScheduler<KernelTraits, true>,
            validation,
            print_layouts_only>(M, N, K, alpha, A, ldA, B, ldB, beta, C, ldC, iters, csv, stream);
        break;
    default:
...
void time_gemm_and_print(Options const& options, bool csv = false)
{
    ...
    dispatch_on_scheduler<TA, TB, TC, TI>(m,
                                          n,
                                          k,
                                          alpha,
                                          d_A.data().get(),
...
void validate(Options const& options)
{
    ...
    dispatch_on_scheduler<TA, TB, TC, TI, true>(m,
                                                n,
                                                k,
                                                alpha,
                                                d_A.data().get(),
    ...
    if (max_abs_diff > eps * size_factor || max_rel_diff > eps)
    {
        ...
        std::cout << "max abs diff = " << max_abs_diff << " at index " << abs_index
                  << ", selected = " << h_C[abs_index] << ", reference = " << h_Cref[abs_index]
                  << std::endl;
...
void print_layouts(Options const& options)
{
    ...
    dispatch_on_scheduler<TA, TB, TC, TI, false, true>(m,
                                                       n,
                                                       k,
                                                       alpha,
                                                       d_A.data().get(),
