...
using GemmConfig
    = std::tuple<int, int, int, int, int, int>; // num_minimal_sms, block_m, block_n, num_stages, num_tma_multicast, best_smem_size
...
int get_num_sms()
{
    ...
    if (num_device_sms == -1)
    {
        ...
        {
            throw std::runtime_error("Failed to get CUDA device" + std::string(cudaGetErrorString(error)));
        }
...
std::string gemm_type_to_string(deep_gemm::GemmType gemm_type)
{
    switch (gemm_type)
    {
    case deep_gemm::GemmType::PerTensorGEMM: return std::string("PerTensorGEMM");
...
GemmConfig get_best_gemm_config(uint32_t shape_m,
                                uint32_t shape_n,
                                uint32_t shape_k,
                                int      num_groups,
                                int      num_device_sms,
                                bool     is_grouped_contiguous = false)
{
    ...
    // 计算所需要wave 数，依据(block_m, block_n)对输入的shape_m和shape_n和num_groups进行切块。
    ...
    // 计算最小SMs需求，主要用于当Wave Quantization问题发生时，对任务重新进行分配
    // 在保证Wave数量不增多的情况下，使用最小的SMs数量完成计算
    ...
    return std::make_tuple(num_minimal_sms, best_block_m, best_block_n, best_num_stages, best_num_tma_multicast, best_smem_size);
}
} // namespace deep_gemm::jit
