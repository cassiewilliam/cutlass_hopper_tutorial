...
template <uint32_t SHAPE_N,
          ...
          typename SchedulerType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
fp8_pef_tensor_quant_gemm_kernel(half*                               gmem_d,
                                 float                               scale,
                                 typename SchedulerType::Input       problem_input,
                                 ...
                                 __grid_constant__ const CUtensorMap tensor_map_d)
{
    ...
    smem_ptr = reinterpret_cast<float*>(smem_buffer + SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE));

    // Fill barriers
    auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_ptr));
    #pragma unroll
    for (int i = 0; i < kNumStages; ++i)
    {
        full_barriers[i] = barrier_start_ptr + i;
...
public:
    ...
    static void run(half*              gmem_d, 
                    float              scale,
                    LayoutIndexType*   grouped_layout,
                    uint32_t           shape_m,
                    CUtensorMap const& tma_a_desc,
                    CUtensorMap const& tma_b_desc,
                    CUtensorMap const& tma_d_desc,
                    cudaStream_t       stream,
                    int                num_sms,
                    uint32_t           smem_size)
    {
        ...
        auto kernel = fp8_pef_tensor_quant_gemm_kernel<SHAPE_N, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K, kNumGroups, kNumStages,
        ...
        auto status = cudaLaunchKernelEx(
            &config, kernel, gmem_d, scale, input, tma_a_desc, tma_b_desc, tma_d_desc);
    ...
    {
        return make_2d_tma_desc(global_address,
                                Layout::ColMajor,
                                SHAPE_K,
                                SHAPE_N * ((kGemmType != GemmType::Normal) || (kGemmType != GemmType::PerTensorQuant) ? kNumGroups : 1), 
                                BLOCK_K,
                                BLOCK_N,
                                global_stride_in_bytes);
    }
    ...
    // NOTE: for per tensor quant gemm
    template <typename LayoutIndexType>
    static void runGemm(void* mat_a, int ld_a, void* mat_b, int ld_b, half* mat_d, int ld_d, float* scale_a,
        float* scale_b, uint32_t shape_m, LayoutIndexType* grouped_layout, cudaStream_t stream, int num_sms,
        uint32_t smem_size)
    {
        ...
        auto tma_d_desc = make_2d_tma_d_desc(reinterpret_cast<half *>(mat_d), shape_m, ld_d * 2);
